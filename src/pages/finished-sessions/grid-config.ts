...
import FinishedSessionRowActions from './row-actions.vue'

export const finishedSessionStatuses = ['Finished', 'EndIdentified', 'EndIdentificationOverdue', 'SendToPayOverdue']

export const finishedSessionPoints: MeasurementPoint[] = ['Start', 'Hour1', 'Hour2', 'Hour3', 'Hour4', 'End']

export function canEndIdentifyRow(row: SessionGridRow): boolean {
  if (row.status !== 'Finished' && row.status !== 'EndIdentificationOverdue') {
    return false
  }

  const filled = row.filledMeasurementPoints
  if (filled) {
    return row.hasEnd === true && filled.includes('End')
  }

  return true
}

export const finishedSessionColumnLabels: Record<string, string> = {
  id: 'ID',
  patientName: 'Пациент',
...
export const finishedSessionDefaultSorting: SortingState = [
  { id: 'identifiedAt', desc: true },
]

export const finishedSessionDefaultColumnPinning: ColumnPinningState = {
  left: ['select', 'id'],
  right: ['actions'],
}

export function createFinishedSessionColumns(opts: {
  tenantNameMap: Record<string, string>
  onEndIdentify: (row: SessionGridRow) => void
  onSendToPay: (row: SessionGridRow) => void
  ...
  return [
    ...
    },
    {
      ...
      cell: ({ row }) => h(MeasurementPointCell, {
        ...
        points: finishedSessionPoints,
        canEdit: !['EndIdentificationOverdue', 'SendToPayOverdue'].includes(row.original.status)
          || opts.canOverrideTimeLimits,
    ...
    },
    {
      ...
      cell: ({ row }) => h(FinishedSessionRowActions, {
        session: row.original,
        canEndIdentify: opts.canEndIdentify
          && canEndIdentifyRow(row.original)
          && (row.original.status !== 'EndIdentificationOverdue' || opts.canOverrideTimeLimits),
        canSendToPay: opts.canSendToPay
          && (row.original.status !== 'SendToPayOverdue' || opts.canOverrideTimeLimits),
        canOverrideTimeLimits: opts.canOverrideTimeLimits,
        canArchive: opts.canArchive
          && (!['EndIdentificationOverdue', 'SendToPayOverdue'].includes(row.original.status) || opts.canOverrideTimeLimits),
        onEndIdentify: () => opts.onEndIdentify(row.original),
        onSendToPay: () => opts.onSendToPay(row.original),
...
export const finishedSessionFilterFields = [
  {
    field: 'status',
    label: 'Статус',
    ...
    options: [
      ...
      { value: 'EndIdentified', label: 'Идент. завершена' },
...
export const finishedSessionExportColumns: ServerDataGridExportColumn<SessionGridRow>[] = [
