...
<template>
  <div class="w-full overflow-hidden">
    <!-- Корневой уровень: показываем все группы -->
      ...
      <UiSidebarGroup v-for="group in navMain" :key="group.title">
        ...
        <UiSidebarMenu v-auto-animate>
          <template v-for="menu in group.items" :key="menu.title">
            ...
            <!-- Родительский пункт: клик для перехода на следующий уровень -->
    ...
    <!-- Вложенный уровень: показываем кнопку назад и пункты текущего уровня -->
    <div v-else :key="`nested-${navigationPath.join('-')}`">
      <UiSidebarGroup>
        <!-- Пункты меню -->
        <UiSidebarMenu v-auto-animate>
          <!-- Заголовок навигации с кнопкой назад -->
          <UiSidebarMenuItem>
            <UiSidebarMenuButton
